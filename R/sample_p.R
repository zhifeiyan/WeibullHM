...
#' 
#' @param p_all a matrix of current update of transition matrices of state process, 
#'        transition matrices under different environments are stacked vertically
...
#'  \item{accept}{a binary indicator vector of acceptance or rejection of proposed 
#'                sample of each row of transition matrices in the independent
#'                Metropolis–Hastings algorithm}
#' @export
sample_p <- function(p_all, q, prior_mat, et, st, ntrial, nstate){
  p_array <- aperm(array(t(p_all), c(nstate, nstate, nstate)), c(2, 1, 3))
  s_stat_old <- stat_dist(p_array, q, nstate)$s_stat
  p_all_temp <- p_all
  accept_vec <- rep(0, nstate ^ 2)
  et_st_id <- (nstate * et + st)[-ntrial]
  
  # Update each of the rows in all p matrices sequentially
  for (k in 0:(nstate ^ 2 - 1)){
    alpha <- prior_mat[k + 1, ]
    id <- which(et_st_id == k)
    if(length(id) == 0){
      n_vec <- rep(0, nstate)
    ...
    p_array_temp <- aperm(array(t(p_all_temp), c(nstate, nstate, nstate)), 
                          c(2, 1, 3))
    s_stat_new <- stat_dist(p_array_temp, q, nstate)$s_stat
